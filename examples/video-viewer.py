...
def main(args, options):
    ...
    video_source = VideoSource(
        args.input,
        width=options.get("input_width", 1280),
        height=options.get("input_height", 720),
        codec=options.get("codec", "mjpg"),
        framerate=options.get("framerate", 10),
    )
    video_output = VideoOutput(
        args.output,
        width=options.get("output_width", 1280),
        height=options.get("output_height", 720),
        framerate=options.get("framerate", 10),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input",
        type=str,
        help="Input URI for the video source (e.g., /dev/video0, rtsp://..., etc.)",
    )
    parser.add_argument(
        "output",
        type=str,
        help="Output URI for the video stream (e.g., display://0, tcp://..., rtsp://...)",
    )
